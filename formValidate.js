...
function validate(e)
{
    ...
    // When using onSubmit="validate()" in markup, return true would allow
    // the form to submit
    return true;
}
...
/*
 * Handles all the error checking for the form.
 *
...
function formHasErrors() {
    ...
    let fields = ["fullname", "address", "city", "postal", "email"];

    for(let i= 0; i< fields.length; i++) {
        let textField = document.getElementById(fields[i]);
        if(textField.ariaValueMax.trim() == "" || textField.value == null) {
    ...
    for(let i=0; i < cardType.length; i++) {
        if(document.getElementById(cartType[i]).checked) {
    ...
    let expiredMonth = document.getElementById("month").value;
    if(expiredMonth == "- Month - ") {
        document.getElementById("month_error").style.display = "block";
        errorFlag = true;
    }

    let expiredYear = document.getElementById("year").value;
    let currentYear = new Date().getFullYear();
    let currentMonth = new Date().getMonth() + 1;

    if(parseInt(expiredYear) < currentYear || ((parseInt(expiredYear) == currentYear &&
        parseInt(expiredMonth) < currentMonth))) {
    ...
    for(let i = 0; i < cardNumber.length-1 ; i++) {
        sum += parseInt(cardNumber[i]) * checkingFactors[i];
    }

    let remainder = sum % 11;
    let checkDigit = 11 - remainder;

    if(!regex.test(cardNumber) || checkDigit !== parseInt(cardNumber[cardNumber.length - 1])) {
	...
	if (!isNaN(quantityValue) && quantityValue != "" && quantityValue != null && quantityValue != 0 && !document.getElementById("cartItem" + itemNumber)) {
		...
		cartItemRemoveButton.addEventListener("click",
			...
			function () {
				...
				// Get a reference to quantity field of the item being removed form the cart
